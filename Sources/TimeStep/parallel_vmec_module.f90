...
CONTAINS
  ...
  !--------------------------------------------------------------------------
    SUBROUTINE InitializeParallel(EXT_MPI_COMM)
      INTEGER, INTENT(IN), OPTIONAL :: EXT_MPI_COMM
   
#if defined(MPI_OPT)
      IF (PRESENT(EXT_MPI_COMM)) THEN
       CALL MPI_Comm_rank(EXT_MPI_COMM,grank,MPI_ERR)
       CALL MPI_Comm_size(EXT_MPI_COMM,gnranks,MPI_ERR)
      ELSE
